...
int readInput(vector<int>& A, int& size) {
...
bool Sort::testIfSorted(vector<int>& A, int size)
{
...
double getAvgSortTime(Option& op, Sort* s, vector<int>&  A, int size)
{
...
int main(int argc, char** argv)
{
  ...
  /* read input integers */
  vector<int> A(size); //stores integers  
  if (readInput(A,size)) 
    return 1; //exit abnormally
